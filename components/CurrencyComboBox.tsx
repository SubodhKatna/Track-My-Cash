      ...
      <SkeletonWrapper isLoading={userSettings.isFetching}>
        <Popover open={open} onOpenChange={setOpen}>
          ...
          <PopoverContent className="w-[200px] p-0" align="start">
            <MemoizedOptionList setOpen={setOpen} setSelectedOption={selectOption} />
          </PopoverContent>
        </Popover>
      </SkeletonWrapper>
    );
...
const OptionList = ({ setOpen, setSelectedOption }: { setOpen: (open: boolean) => void; setSelectedOption: (status: Currency | null) => void }) => {
  return (
    <Command>
        ...
        <CommandGroup>
          {Currencies.map(currency => (
            <CommandItem
              ...
              onSelect={value => {
                setSelectedOption(Currencies.find(c => c.value === value) || null);
                setOpen(false);
              }}
            >
              {currency.label}
