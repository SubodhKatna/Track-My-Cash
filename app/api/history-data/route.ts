...
export async function GET(request: Request) {
  ...
  const querParams = getHistoryDataSchema.safeParse({
    timeframe,
    month,
    year,
  });
  ...
  if (!querParams.success) {
    return Response.json(querParams.error.message, {
      status: 400,
    });
  }
  const data = await getHistoryData(user.id, querParams.data.timeframe, {
    month: querParams.data.month,
    year: querParams.data.year,
  });
...
async function getHistoryData(userId: string, timeframe: Timeframe, period: Period) {
  switch (timeframe) {
    ...
    case 'month':
      return await getMonthHistoryData(userId, period.year, period.month ?? 0);
  }
}
